...
> ⚠️ **Frontmatter fields**: `name` and `description` are required. Cross-tool fields: `argument-hint`, `user-invocable`, `disable-model-invocation`, `allowed-tools`. Copilot CLI + Claude Code: `model` (v1.0.22+). Claude Code-only: `effort`, `context`, `agent`, `hooks`. Copilot CLI also supports `license`. Unknown fields are silently ignored. See [references/skill-patterns.md](references/skill-patterns.md#frontmatter-required) for the full cross-tool compatibility table.
